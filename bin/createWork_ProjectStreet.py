...
def analyse():
    if os.listdir(RawData):
            ...
            oldest = files[0]
            last_wu_results = args.last_wu_results  #needs to be as []
            ...
            if last_stage:
                if last_stage == "1"
                    ...
                    os.system(boinc2docker + '--rsc_fpops_est 90000e10 --delay_bound 1.21e+6 mtrnord/projectstreet_detection:' + last_wu_id + ' latest sh -c "echo "2" >> /root/shared/results/stage.txt && ./stage2_generateHaarDetector.sh 2>&1 | tee /root/shared/results/logs.txt"')
                if last_stage == "2"
                    addFiles(last_wu_results, last_wu_id)
                    os.system(boinc2docker + '--rsc_fpops_est 90000e10 --delay_bound 1.21e+6 mtrnord/projectstreet_detection:' + last_wu_id + ' latest sh -c "echo "3" >> /root/shared/results/stage.txt && ./stage3_analyseVideo.py 2>&1 | tee /root/shared/results/logs.txt"')
            else:
                addFiles(None, hash, os.path.join(RawData, oldest))
                os.system(boinc2docker + '--rsc_fpops_est 90000e10 --delay_bound 1.21e+6 mtrnord/projectstreet_detection:' + last_wu_id + ' latest sh -c "echo "1" >> /root/shared/results/stage.txt && ./stage2_generateHaarDetector.sh 2>&1 | tee /root/shared/results/logs.txt"')
